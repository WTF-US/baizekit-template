...
pub async fn get_{{entity}}s(
    ...
    state.{{entity}}.search(cmd).await.map(ApiOK::with_data).map_err(ApiError)
}
...
pub async fn post_{{entity}}s(State(state): State<AppState>, Json(cmd): Json<AddCommand>) -> ApiResult<{{entity | pascal_case}}DTO> {
    state.{{entity}}.add(cmd).await.map(ApiOK::with_data).map_err(ApiError)
}
...
pub async fn put_{{entity}}(
    ...
    cmd.id = id;
    state.{{entity}}.update(cmd).await.map(ApiOK::with_data).map_err(ApiError)
}
